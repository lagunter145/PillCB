...
void print_clock(int time, int number) {
    ...
    if(number == 1) {
        write_to_display(s1, 100);
    }
    if(number == 2) {
        write_to_display(s1, 70);
    }
    if(number == 3) {
        write_to_display(s1, 50);
    }

    //LCD_DrawString(90,100, WHITE, WHITE, s1, 16, 1);
}
void write_to_display(char* s1, int pos) {
    LCD_DrawString(90, pos, WHITE, WHITE, s1, 16, 1);
}
